...
<img src="@if($data->{$row->field}==null){{ Voyager::image( setting('site.product_def_image') ) }}@elseif( !filter_var($data->{$row->field}, FILTER_VALIDATE_URL)){{ Voyager::image( $data->{$row->field} ) }}@else{{ $data->{$row->field} }}@endif" style="height:50px">
                                                ...
                                                @elseif(($row->type == 'date' || $row->type == 'timestamp') && !is_null($data->{$row->field})  )

                                                    @if($row->type == 'date')
                                                        {{ convertSystemTzToUserTz($data->{$row->field},'Y-m-d','m/d/Y') }}
                                                    @elseif($row->type == 'timestamp')
                                                        {{ convertSystemTzToUserTz($data->{$row->field}) }}
                                                    @else
                                                        {{ $data->{$row->field} }}
                                                    @endif
                                                ...
                                                @elseif($row->type == 'number')
                                                    @if(property_exists($row->details, 'currency'))
                                                        @if($data->{$row->field})
