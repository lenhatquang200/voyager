    ...
    public function index(Request $request)
    {
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            if ($model && in_array(SoftDeletes::class, class_uses_recursive($model))
                && Auth::user()->can(
                    'delete', app($dataType->model_name)
                )
            ) {
...
//            $dataRow = Voyager::model('DataRow')->whereDataTypeId($dataType->id)->get();
            foreach ($dataType->browseRows as $row) {
                if ($row->field !== null && ($row->type == 'text' || $row->type == 'number' || $row->type == 'time'
                        || $row->type == 'timestamp')) {
                    $displayName = $row->getTranslatedAttribute('display_name');

                    $searchNames[$row->field] = $displayName;
                }
            }
        }
    ...
    public function relation(Request $request)
    {
        ...
        foreach ($rows as $key => $row) {
            if ($row->field === $request->input('type')) {
                ...
                if (isset($options->scope) && $options->scope != ''
                    && method_exists(
                        $model, 'scope'.ucfirst($options->scope)
                    )
                ) {
                ...
                if ($search) {
                    ...
                    if (in_array($options->label, $additional_attributes)) {
                        ...
                        $relationshipOptions = $relationshipOptions->filter(
                            function ($model) use ($search, $options) {
                ...
                if ((!$row->required && !(isset($row->details) && isset($row->details->required))) && !$search
                    && $page == 1
                ) {
                    $results[] = [
                        'id'   => '',
